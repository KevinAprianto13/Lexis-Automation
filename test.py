...
def loop_frequencies(ip_list, file_path):
    ...
    print(f"Frekuensi yang digunakan: {frequencies}")  # Debug

# Fungsi untuk looping frekuensi 1, 6, 11 dan menyimpannya ke frequencies.txt
def loop_frequencies_with_1_6_11(ip_list, file_path="frequencies.txt"):
    channel_to_frequency = [2412, 2437, 2462]  # Frekuensi 1, 6, 11
    frequencies = {}
    ...
    # Simpan hasil ke frequencies.txt
    with open(file_path, "w") as freq_file:
        for ip, frequency in frequencies.items():
            freq_file.write(f"{ip}:{frequency}\n")
...
def schedule_for_ips(ip_list, time_str, config_function, *args):
    for ip in ip_list:
        if args:
            config_commands = config_function(ip, *args)
        else:
            config_commands = config_function(ip)
        schedule.every().day.at(time_str).do(execute_ip_config, ip, config_commands)
        print(f"1 tugas telah dijadwalkan. Menunggu eksekusi...")
        time.sleep(1)

# Fungsi untuk eksekusi segera
def execute_now(ip_list, config_function, *args):
    for ip in ip_list:
        if ping_ip(ip):
            config_commands = config_function(ip, *args)
...
def handle_frequency_config():
    ...
    if ip_choice == "1":
        ...
        loop_frequencies(ip_addresses, "frequencies.txt")  # Panggil fungsi looping biasa
        frequencies = load_frequencies("frequencies.txt")  # Muat frekuensi yang sudah di-generate
        ...
        while True:
            loop_frequencies(ip_addresses, "frequencies.txt")
            frequencies = load_frequencies("frequencies.txt")
            time.sleep(5)  # Tunggu beberapa detik sebelum looping lagi
            
            # Tanya pengguna jika ingin melanjutkan
        ...
        print(f"Frekuensi acak yang dihasilkan: {frequencies}")
    ...
    elif ip_choice == "5":  # Opsi baru untuk loop frekuensi 1, 6, 11
        frequencies = loop_frequencies_with_1_6_11(ip_addresses, "frequencies.txt")
        print(f"Frekuensi 1, 6, 11 telah di-generate: {frequencies}")
    ...
    while True:
        try:
            ...
            if config_type == "1":
                ...
                print("Jenis konfigurasi tidak valid.")
